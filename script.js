...
// --- 4. SETUP SCREEN LOGIC (With Duplicate Checker) ---
btnStart.addEventListener('click', () => {
    const targetDate = datePicker.value;
    const targetBatch = batchSelect.value;

    // 1. Grab your local history
    const logVault = JSON.parse(localStorage.getItem('attendanceAuditLog')) || [];
    const offlineVault = JSON.parse(localStorage.getItem('offlineAttendanceVault')) || [];

    // 2. Search for a match
    const alreadySubmitted = logVault.find(entry => 
        entry.batch === targetBatch && 
        entry.date === targetDate && 
        entry.status.includes('Submitted')
    );

    const alreadyOffline = offlineVault.find(entry => 
        entry.batch === targetBatch && 
        entry.date === targetDate
    );

    // 3. Trigger the Warning
    if (alreadySubmitted || alreadyOffline) {
        const time = alreadySubmitted ? alreadySubmitted.time : "Pending Sync";
        const msg = `⚠️ DUPLICATE DETECTED!\n\nAttendance for ${targetBatch} on ${targetDate} was already recorded (${time}).\n\nDo you want to take it again anyway?`;
        
        if (!confirm(msg)) return; // If they click 'Cancel', the app stops here.
    }

    // 4. If they click 'OK' or no duplicate is found, proceed as usual
    sessionData.date = targetDate;
    sessionData.batch = targetBatch;
...
function renderReview() {
    ...
    currentBatchList.forEach((student, index) => {
        // --- THE MAGIC LINE: Extract last 3 digits AND remove leading zeros ---
        // parseInt("045") becomes 45 | parseInt("001") becomes 1 | parseInt("102") stays 102
        const rollNo = parseInt(student.enrollment.slice(-3), 10);
        ...
        // Now shows the clean number (1, 45, 100, etc.)
        box.innerText = rollNo; 
        ...
        // Updates the list view with the clean number as well
        listItem.innerHTML = `<span>${rollNo}. ${student.name}</span> <span style="color: ${statusColor};">${status}</span>`;
...
// --- 9. CLOUD PULL LOGIC (The Receiver) ---
async function pullConfigFromCloud() {
    // Only pull if the local vault is empty (to save data/speed)
    const localStudents = localStorage.getItem('studentVault');
    
    if (!localStudents || localStudents === "{}") {
        console.log("Local vault empty. Pulling from Cloud...");
        
        try {
            const response = await fetch(GOOGLE_APP_URL, {
                method: 'POST',
                body: JSON.stringify({ action: "pullConfig" })
            });
            const data = await response.json();
            
            if (data.status === 'success') {
                localStorage.setItem('adminVault', data.teachers);
                localStorage.setItem('studentVault', data.students);
                
                // Refresh the list if they are already on the setup screen
                alert("Cloud Sync Complete: Teacher and Student records updated!");
                location.reload(); 
            }
        } catch (err) {
